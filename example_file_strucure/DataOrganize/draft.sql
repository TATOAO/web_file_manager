set hivevar:started_date=date_sub(current_date(), 90);
set hivevar:end_date=date_sub(current_date(), 2);
select
msg_table.* ,
active_table.* ,
case when 
	active_table.active_date is not null then 1 
	else 0 end 
	as is_huoyue,
case when 
	active_table.active_date is not null and msg_table.clk_tms > 0 then 1 
	else 0 
...
from CX_HCZAPP_SAFE.DMD_ONSA_SGT_DYNA.SHRT_CLK_D
...
(
select *
	from cx_dwf_safe.his_dwe_cus_insurance_xs_attr_d_cc
...
on user_label.mobilephone = msg_table.mb_no and msg_table.send_minus_2_date = user_label.data_date
...
msg_table.stat_dt=active_table.active_date
order by rand_index
